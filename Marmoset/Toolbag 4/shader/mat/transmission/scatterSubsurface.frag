...
uniform vec4	uFuzzColor;
uniform vec4	uTranslucencyColor;
uniform float	uTranslucencySky;
...
void	sampleScatterParams( inout FragmentState s, inout ScatterParams scatter ) 
{
	...
	vec4 translucencyMap =	vec4(1.0,1.0,1.0,1.0);
	vec4 fuzzMap =			textureMaterial( tFuzzMap, s.vertexTexCoord );	

	scatter.subdermisColor =		uScatterColor.rgb * scatterMap.rgb; //TODO: consider putting albedo into subdermis color for extra physics
	scatter.scatterDepth =			uScatterDepth / uMillimeterScale; //scatter depth in world coordinates

	scatter.translucencyColor =		uTranslucencyColor.rgb * translucencyMap.rgb;
	scatter.translucencyScatter =	uTranslucencyScatter;
	scatter.translucencyDepth =		uTranslucencyDepth;
	scatter.translucencyMask =		max(max(scatter.translucencyColor.r,scatter.translucencyColor.g),scatter.translucencyColor.b) * uTranslucencyColor.a;

	scatter.fuzzColor = 			uFuzzColor * fuzzMap;
	scatter.fuzzScatter =			0.3;	
	...
	float mask = max(scatter.subdermisColor.r, max(scatter.subdermisColor.g, scatter.subdermisColor.b));
	mask = 1.0 - mask;
	mask *= mask;
	mask *= mask;
	mask *= mask;	
	mask = 1.0 - (mask*mask);
	scatter.mask = uScatterColor.a * scatterMap.a * mask;

	s.scatterColor = scatter.subdermisColor; //needed for PaintMerge
}

void	DiffusionScatterEnv( inout FragmentState s, inout DiffusionParams scatter )
{
	sampleScatterParams( s, scatter );
