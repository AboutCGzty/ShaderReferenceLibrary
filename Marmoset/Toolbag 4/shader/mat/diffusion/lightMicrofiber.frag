...
void	DiffusionMicrofiberEnv( inout FragmentState s )
{
	...
	//specular light
	{
		...
		s.specularLight += spec * (1.0/float(SHEEN_IMPORTANCE_SAMPLES)) * uSheenReflectionBrightness;
	}
...
void	DiffusionMicrofiberLight( inout FragmentState s, LightParams l )
{
	...
	//specular light
	{
		//TODO: derive proper normalization factor ~ms
		LightParams lspec = l;
		vec3 R = reflect( -s.vertexEye, s.normal );
		adjustAreaLightSpecular( lspec, R, 1.0 );

		vec3  L = lspec.direction;
		vec3  H = normalize( L + s.vertexEye );
		...
		vec3  spec  = lspec.color * lspec.shadow.rgb * lspec.attenuation;
		float denom = 4.0 * NdotV; //non cancelled-out microfacet BRDF normalization terms
		s.specularLight += spec * F * D * G * ( denom > 0.0 ? rcp(denom) : 0.0 );
	}
	...
	//diffuse light
	{
		...
		float specAlbedoL = texture2D( tMicrofiberSheenAlbedoLUT, vec2(NdotL, roughness) ).x;
		float specAlbedoV = texture2D( tMicrofiberSheenAlbedoLUT, vec2(NdotV, roughness) ).x;
		vec3  specAlbedo  = min( oneminus(s.sheen * specAlbedoL), oneminus(s.sheen * specAlbedoV) );

		vec3 diffuse = l.color * l.shadow.rgb * l.attenuation * NdotL;
		s.diffuseLight += diffuse * specAlbedo * s.albedo.rgb * INVPI;
	}
}
