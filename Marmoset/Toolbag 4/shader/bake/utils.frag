...
vec3	findTraceOrigin( vec3 position, vec3 direction, vec2 uv )
{
	...
	offset = mix( uTraceOffsetRange.x, uTraceOffsetRange.y, offset );

	vec3 origin = position - offset * direction;
	return origin;
}

vec3	findTraceOriginWithCustomCage( vec3 position, vec3 direction, vec3 cagePosition )
{
	//custom cage position is really just used to specify an offset magnitude
	//(we still want the ray to pass through the low poly position) - ag & jdr
	float offset = length( position - cagePosition );
