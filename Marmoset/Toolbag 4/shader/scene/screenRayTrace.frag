...
{
	#define SSR_STEPS			64
	#define SSR_REFINE_STEPS	16

	//ray march settings
	float maxRayDist = abs(pos.z)*rsqrt(0.01 + saturate(dot(dir.xy,dir.xy)));
	float incr = maxRayDist/float(SSR_STEPS);
	
	bool under = false, hit = false;
	vec2 tc;
	float edge = 0.0;
	HINT_LOOP
	for( float t=incr; t<=maxRayDist && !hit && edge < 1.0; t+=incr )
	{
		...
		if( !wasUnder && under )
		{
			...
			{
				float t2 = t - (1.0 - float(i+1)/float(SSR_REFINE_STEPS+1))*incr;
				vec3 p2 = pos + t2*dir;
