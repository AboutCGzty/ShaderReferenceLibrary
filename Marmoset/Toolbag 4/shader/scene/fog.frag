...
uniform mat4	uLightMatrix;
	...
	uniform vec4	uLightPosition;		// { x, y, z, directional ? 0 : 1 }
	uniform vec3	uLightColor;		// { r, g, b }
	...
	vec3 color = uFogColor;
		...
		return;
			...
			float t0 = 0.0, t1 = 0.0;
			{
				...
				float a = 1.0;
				float b = 2.0 * dot( worldEye, startPos - uLightPosition.xyz );
				float c =	dot( uLightPosition.xyz, uLightPosition.xyz ) +
							dot( startPos, startPos ) +
							-2.0 * dot( uLightPosition.xyz, startPos ) +
							- r*r;
				float root = b*b - 4.0*a*c;
				if( root >= 0.0 )
				{
			...
			#if defined(FOG_SPOT)
			{
				...
				vec3 dp = startPos - uLightPosition.xyz;
				...
				if( root >= 0.0 )
				{
					...
					bool c0behind = dot( -uLightPosition.xyz + startPos + worldEye*c0, uSpotParams.xyz ) <= 0.0;
					bool c1behind = dot( -uLightPosition.xyz + startPos + worldEye*c1, uSpotParams.xyz ) <= 0.0;
			...
			if( t1 > t0 )
			{
				...
				for( int i=0; i<SAMPLES; ++i )
				{
					...
					vec3 lightToPoint = p - uLightPosition.xyz;
			...
			color *= hit * uFogIllum * uLightColor;
			...
			color *= (0.1 * f) * (disp * uFogIllum) * uLightColor;
