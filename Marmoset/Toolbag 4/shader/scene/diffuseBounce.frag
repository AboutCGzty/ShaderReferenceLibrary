...
END_PARAMS
{
	//position & eye vector
	vec3 position;
	position.z = texture2DLod( tScreenDepth, fCoord, 0.0 ).x;
	position.xy = position.z * ( fCoord * uUnproject.xy + uUnproject.zw );
	...
	if( fade > 0.0 )
	{
		...
		{
			vec3 hitPosition;
			hitPosition.z = texture2DLod( tScreenDepth, hitCoords, 0.0 ).x;
			hitPosition.xy = hitPosition.z * ( hitCoords * uUnproject.xy + uUnproject.zw );
