...
vec4	sampleHit( vec2 coord, vec2 offset, ShadeParams p )
{
	...
	if( tc.x > 0.0 || tc.y > 0.0 )
	{
		...
		vec3 hit;
		hit.z = texture2DLod( tDepth, tc, 0.0 ).x;
		hit.xy = hit.z * ( tc * uUnproject.xy + uUnproject.zw );
...
END_PARAMS
{
	...
	ShadeParams p;
	p.position.z = texture2DLod( tDepth, fCoord, 0.0 ).x;
	p.position.xy = p.position.z * ( fCoord * uUnproject.xy + uUnproject.zw );
