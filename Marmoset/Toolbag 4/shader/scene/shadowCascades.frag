...
uniform float	uSceneScale;	// 1 / sceneSize
...
END_PARAMS
{
	...
	transScale *= 0.2;						//divide by 5 to match the exponential curve to world units by appearance
		...
		vec2 searchSpaceSize = uAreaScales[cascadeIndex] * lightSize;
		...
		if( occluderCount > 0.0 )
		{
			...
			vec2 pwt = searchSpaceSize * ( (offsetObjDepth-avgOccluder) * uSceneScale );
