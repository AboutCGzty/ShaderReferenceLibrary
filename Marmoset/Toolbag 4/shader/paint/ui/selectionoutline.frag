...

{
	...
	vec4 selectLeft = texture2D(tSelection, texLeft) * float(texLeft.x > 0.0);
	vec4 selectRight = texture2D(tSelection, texRight)  * float(texRight.x < 1.0);
	vec4 selectUp = texture2D(tSelection, texUp) * float(texUp.y < 1.0+tdy * 2.0);	//some fudging here
	vec4 selectDown = texture2D(tSelection, texDown) * float(texDown.y > -tdy * 2.0);	//and here
	float threshold = 0.5;
