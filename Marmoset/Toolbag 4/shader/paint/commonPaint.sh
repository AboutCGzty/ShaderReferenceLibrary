...
float distanceToValue(float dist, float ddr, float brushHardness)
{
...
#ifndef THIN_STROKE
	dist = dist * 0.5 * mix(1.414, 2.0, brushHardness);
	...
	dist = mix(dist * 0.5 * mix(1.414, 2.0, brushHardness), dist, smallness);	//this helps us get a nicer radius for soft brushes	
