...
vec4 getRawTilingData(vec3 pos, vec3 normal)
{
	...
	output.w = 1.0f-(intersection.percentToHit/maxProjectionPercent);
...
vec4 convertToOutput(vec4 rawData)
{
	...
	if( rawData.x == 0 && rawData.y == 0 ) return vec4(1,1,1,1);

	if( uChannel != 10 && uChannel != 12 )//!displacement or bump
	{
		//apply coloring
		if( rawData.x == 0 )
		{
			...
			float shadowValue = 1.0f - ((1.0f-seam)*(1.0f-uBorderShadowDepth));//from 1.0 towards 0.0 scaled by user depth control
			float lerp = 1.0f - seam;
		...
		else
		{
			...
			vec4 smoothBevel = (uBevelColor*(1.0f-bevel))+(uTileColor*bevel);
			outputColor.xyz = (hardBevel.xyz*(1.0f-uBevelRoundness))+(smoothBevel.xyz*uBevelRoundness);
		}
	}
	else
	{
		...
		if( rawData.x == 0 )
		{
			...
			value = (value*(1.0f-lerp))+(shadowValue*lerp);
		}
		outputColor.x = value;
		outputColor.y = value;
		outputColor.z = value;
...
END_PARAMS
{
	...
	vec3 normal = vec3(0.0 ,0.0, 1.0);	
		...
		vec4 rawData;
		rawData = getRawTilingData( getTilingCoords(p.uvX.x, p.uvX.y), normal );
		vec4 outX = convertToOutput(rawData);
		rawData = getRawTilingData( getTilingCoords(p.uvY.x, p.uvY.y), normal );
		vec4 outY = convertToOutput(rawData);
		rawData = getRawTilingData( getTilingCoords(p.uvZ.x, p.uvZ.y), normal );
		vec4 outZ = convertToOutput(rawData);
		outputColor = triplanarMix( p, outX, outY, outZ );
	#else
		x = state.texCoord.x;
		y = state.texCoord.y;
		vec4 rawData = getRawTilingData( getTilingCoords(x,y), normal );
		outputColor = convertToOutput(rawData);
	#endif
