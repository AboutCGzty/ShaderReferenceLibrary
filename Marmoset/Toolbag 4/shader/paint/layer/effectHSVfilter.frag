...
{
	vec4 c = formatInputColor( texture2DLod( tHSVFilter, uv, 0 ) );
...
{
	vec4 sample = texture2DLod( tHSVFilter, uv, 0 );
	...
	sample = formatInputColor( sample );

	sample.w = 1;

	vec3 hsv = RGBToHSV( sample.xyz );
	hsv.x += uHueCycle * 360;
	if( hsv.x > 360 ) hsv.x -= 360;
	sample.xyz = HSVToRGB( hsv );
