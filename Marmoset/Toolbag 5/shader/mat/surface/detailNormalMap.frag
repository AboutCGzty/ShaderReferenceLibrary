...
void	SurfaceDetailNormalMap( in SurfaceDetailNormalMapParams p, inout MaterialState m, inout FragmentState s )
{
	...
	projectTaps( tapX, tapY, tapZ, m.vertexTexCoord.projectorCoord );
	...
	tap = ProcessDetailNormalSample( tap, p.base.flags );
	
	// detail normal map taps are in tangent space, move them to shading space
	dn = tap.x * T + tap.y * B + tap.z * N;
#endif
...
#define Surface(p,m,s)		SurfaceDetailNormalMap(p.surface,m,s)
