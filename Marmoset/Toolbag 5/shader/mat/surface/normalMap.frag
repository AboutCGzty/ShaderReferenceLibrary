...
uniform mat4 uNormalMapShadingSpaceMatrix;
#endif
...
vec3	TransformNormalFromObjectSpaceToShadingSpace( mat3 transformInverseTranspose, vec3 n )
{
	n = mulVec( transformInverseTranspose, n );
	#if defined(MATERIAL_PASS_LIGHT) ||\
	    defined(MATERIAL_PASS_SCATTER) ||\
	    defined(MATERIAL_PASS_COMPONENTVIEW) ||\
	    defined(MATERIAL_PASS_PREPASS) ||\
	    ...
	    defined(MATERIAL_PASS_PREPASS_HYBRID) ||\
	    defined(MATERIAL_PASS_SHADOWMAP)
		//shading space is sky orientation
		mat3 transformShadingSpace = mat3( uNormalMapShadingSpaceMatrix[0].xyz, uNormalMapShadingSpaceMatrix[1].xyz, uNormalMapShadingSpaceMatrix[2].xyz );
		n = mulVec( transformShadingSpace, n );
	#endif
	return n;
}

void	SurfaceNormalMap( in SurfaceNormalMapParams p, inout MaterialState m, inout FragmentState s )
{
	...
	vec4 n = triplanarMix( m.vertexTexCoord.projectorCoord, nX, nY, nZ );
	n.xyz = mulVec( m.vertexTexCoord.projectorToShadingRotation, n.xyz );
#else
	...
	if( p.flags & NORMALMAP_FLAG_OBJECTSPACE )
	{
		n = TransformNormalFromObjectSpaceToShadingSpace( s.transformInverseTranspose, n );
	}
	else
	{
		n = n.x*T + n.y*B + n.z*N;
...
#define	Surface(p,m,s) 			SurfaceNormalMap(p.surface,m,s)
