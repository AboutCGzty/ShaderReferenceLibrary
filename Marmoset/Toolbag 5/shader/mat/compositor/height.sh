...
{
	float blendingCoefficient = textureMaterial( params.texture, state.vertexTexCoordBase, 1.0 );
    blendingCoefficient = params.brightnessContrastScaleBias.x * blendingCoefficient + params.brightnessContrastScaleBias.y;
	blendingCoefficient = saturate( blendingCoefficient * ( params.layerScale ) - layerIndex + 1 );
